...
class MeetingRoom(object):
    ...
    def get_date_time(*args):
        ...
        return date_time_f, meeting_end_f

    def add_meeting(self, *args):
        date_time_obj, meeting_end_obj = self.get_date_time(*args)

        if self.check_overlap(date_time_obj):
            MeetingRoom.appointments[self.name].append(
                [date_time_obj.date(), date_time_obj.time(), meeting_end_obj.time()])

...
class AbstractPayment(metaclass=ABCMeta):
    @abstractmethod
    def authorize_payment(self, user_id:int,
                          amount:int, currency:str) -> bool:
        pass

    @abstractmethod
    def charge_payment(self, user_id:int, amount:int,
                       currency:str, merchant_id:int) -> int:
        pass

    @abstractmethod
    def reverse_payment(self, user_id:int, amount:int,
                        currency:str, merchant_id:int) -> int:
        pass
...
class MyNewPayment(AbstractPayment):
    ...
    def charge_payment(self, user_id:int, amount:int,
                       currency:str, merchant_id:int):
        if amount < 0 or amount > 1000:
            raise CannotChargeError(f"user_id {user_id}, amount {amount}, currency {currency}, merchant_id {merchant_id}")
    ...
    def reverse_payment(self, user_id:int, amount:int,
                        currency:str, merchant_id:int):
        if amount < 0 or amount > 1000:
            raise CannotReverseError(f"user_id {user_id}, amount {amount}, currency {currency}, merchant_id {merchant_id}")
